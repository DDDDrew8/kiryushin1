...
const Navbar = ({ theme, toggleTheme }) => { // theme и toggleTheme не используются в этом фрагменте
  ...
  const navLinks = [
    { name: 'Home', path: '#exercises', icon: AiOutlineHome }, // Путь к #exercises
    { name: 'About', path: '#about', icon: AiOutlineUser },   // Путь к #about
            ...
            {navLinks.map((link) => (
              <motion.li key={link.name} variants={navLinkVariants}>
                <a
                  ...
                  aria-current={activeLink === link.path.substring(1) ? 'page' : undefined} // Обновлено: используем path.substring(1)
                >
                  <div
