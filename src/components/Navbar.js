...
const Navbar = ({ theme, toggleTheme }) => { // theme и toggleTheme не используются в этом фрагменте
  ...
  useEffect(() => {
    const calculateOffsets = () => {
      // Ищем элементы с атрибутом id (включая #exercises и #about)
  ...
  useMotionValueEvent(scrollY, 'change', latest => {
    ...
    for (const section of sectionOffsets) {
      // Смещение для точности, чтобы активная ссылка переключалась чуть раньше
  ...
  const handleSmoothScroll = (e, targetId) => {
    ...
    console.log("handleSmoothScroll вызван для:", targetId);

    e.preventDefault();

    if (targetId === '#home') {
      // Прокрутка к началу страницы (к разделу упражнений)
      // Используем id="exercises", предполагая, что он есть в ExerciseSection
      const exercisesSection = document.querySelector('#exercises');
      if (exercisesSection) {
         // Обновляем активную ссылку перед прокруткой
        setActiveLink('exercises');
        exercisesSection.scrollIntoView({ behavior: 'smooth' });
      } else {
        // Если #exercises нет, прокручиваем к началу
        window.scrollTo({ top: 0, behavior: 'smooth' });
        setActiveLink('home'); // Обновляем активную ссылку
      }
    } else {
      const section = document.querySelector(targetId);
      console.log("Ищем элемент:", targetId, "Найден:", section); // Добавим лог для поиска элемента
      if (section) {
        // Обновляем активную ссылку перед прокруткой
  ...
  // Определение пунктов меню (теперь с якорными ссылками и текстовым элементом)
  const navLinks = [
    { name: 'Home', path: '#exercises', icon: AiOutlineHome, type: 'link' }, // Добавлен тип 'link'
    { name: 'В.В. Кирюшин', type: 'text' }, // Специальный тип для текста
    { name: 'About', path: '#about', icon: AiOutlineUser, type: 'link' },   // Добавлен тип 'link'
    // { name: 'Projects', path: '#projects', icon: AiOutlineProject, type: 'link' }, // Закомментировано
    // { name: 'Studies', path: '#studies', icon: AiOutlineBook, type: 'link' },     // Закомментировано
    // { name: 'Contact', path: '#contact', icon: AiOutlineMail, type: 'link' },     // Закомментировано
  ];
      ...
      >
        <nav className="flex items-center justify-between w-full" aria-label="Main navigation">
          ...
          <ul className="flex items-center justify-between w-full px-1 md:px-2">
            {navLinks.map((link, index) => ( // Добавим index для уникального ключа
              <React.Fragment key={index}> {/* Используем React.Fragment для группировки */}
                {link.type === 'link' ? (
                  // Рендерим обычную ссылку
                  ...
                  // Рендерим текстовый элемент
                  <motion.li variants={navLinkVariants} className="flex flex-col items-center">
                    <div className="flex flex-col items-center">
                      {/* Используем div вместо a, так как это не ссылка */}
                      <div className="p-2 md:p-2.5 rounded-xl text-white/80">
                        <span className="text-[9px] md:text-[10px] font-medium">
                          {link.name}
                        </span>
                      </div>
                      {/* Не показываем индикатор для текста */}
                    </div>
                  </motion.li>
                ) : null} {/* Если тип не 'link' и не 'text', ничего не рендерим */}
              </React.Fragment>
            ))}
          </ul>
        </nav>
